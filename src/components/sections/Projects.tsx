  ...
  const getCategoryLabel = (cat: string) => {
    switch (cat) {
      case 'residential':
        return 'Rodinné domy';
      case 'renovations':
        return 'Rezidenčné budovy';
      case 'commercial':
        return 'Komerčná výstavba';
      case 'industrial':
        return 'Priemyselné objekty';
      case 'civil':
        return 'Občianské stavby';
      default:
        return cat;
    }
  };
        ...
        {!hideFilters && (
          <div className="flex flex-col gap-4 mb-10 border-b border-gray-100 pb-6 overflow-hidden">
            ...
            <div className="flex overflow-x-auto gap-2 pb-2 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
              {(['all', 'residential', 'renovations', 'commercial', 'industrial', 'civil'] as const).map((cat) => (
                <button
                  key={cat}
                ...
                >
                  {cat === 'all' ? ('Všetky realizácie') : getCategoryLabel(cat)}
                </button>
              ))}
            </div>
